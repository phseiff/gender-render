"""
The following code allows checking if an object matches a specific type annotation provided by the typing module.
It is used at various places in my automated tests.

The code is directly taken with only minor modifications from the following stack overflow answer:
   https://stackoverflow.com/a/55504010/9816158,
which is licensed via CC BY-SA 4.0 (https://creativecommons.org/licenses/by-sa/4.0/).

It was written by stack overflow user Aran-Fey (https://stackoverflow.com/users/1222951/aran-fey) as an answer to
question https://stackoverflow.com/questions/55503673/how-do-i-check-if-a-value-matches-a-type-in-python.
"""
...
def is_instance(obj, type_):
    ...
    # This codeblock is my own, phseiff's, addition:
    if isinstance(type_, typing_extensions._Literal):
        return obj in type_.__values__

    # This codeblock is my own, phseiff's, addition:
    if type(type_) is typing_extensions._TypedDictMeta:
        obj = copy.deepcopy(obj)
        # print(str(obj)[:100], type_)
        if not type(obj) is dict:
            # print("not a dict.")
            return False
        # print(type_.__annotations__)
        for name in obj:
            if name not in type_.__annotations__:
                return False
        for name, type__ in type_.__annotations__.items():
            if name not in obj:
                obj[name] = type(None)()
                # print("adding", name, "to obj.")
            if not is_instance(obj[name], type__):
                # print("is not instance:", name, obj[name], type__)
                break
            else:
                pass
                # print("is instance:", name, type__)
        else:  # <- we didn't break due to finding smth wrong
            return True
        return False
